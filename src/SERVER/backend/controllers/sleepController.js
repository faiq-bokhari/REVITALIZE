...
const addSleepData = async (req, res) => {
  ...
  if (!sleepHour || !bedHour || !sleepMinute || !bedMinute || !lowerCaseEmail || !dateAdded) {
    res
      .status(400)
      .json({ success: false, message: "Please fill in all fields for addSleep" });
    return;
  }
...
const getSleepData = async (req, res) => { 
  ...
  try {
      ...
      res.status(404).json({ message: "Error in getting sleep data" });
  }
}
...
const getAllUserSleepData = async (req, res) => {  
  ...
  try {
      ...
      res.status(200).json(allUserSleepDatas);
  } catch (error) {    
...
const deleteSleepData = async (req, res) => {
  ...
  try {
    ...
    console.log("DELETED SLEEP DATA", deleteSleepData);
    if (deletedSleepData) {
      res.status(200).json(deletedSleepData);
    } else {
