...
\begin{description}
	\item[\refstepcounter{acnum} \actheacnum \label{acHardware}:] The format the date is stored in
	\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] The labels of buttons to navigate screens
	\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] Colouring of screens
	\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] Input text box length and location
	\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] Layout of calendar
	\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] Nutrition display format
	\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] Recipe search criteria
	\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] Recipe query result format
	\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] Custom meal input format
	\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] Workout display format
	\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] Exercise list search criteria
	\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] Exercise query result format
	\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] Sleep time input (sensitivity of circular scroll bar)
\end{description}
